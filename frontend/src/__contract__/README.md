...
The nine `chat_*`, `admin_*` and `session.json` files of M3, and the two
`kernel_*` files of M4, were emitted by the API test harness for the same reason,
and are regenerated by a test rather than by curl:
...
and a chat session with history, or a JupyterHub with a running pod, which is more
setup than a capture script should own — and emitting them from the handlers is the
same guarantee, since it is still the backend marshalling its own types. The M4 pair
comes from the in-memory Hub in `pkg/kernel/kerneltest`, so the kernel ids and the
one listed file are a fake's.
