...
The nine `chat_*`, `admin_*` and `session.json` files of M3 were emitted by the API
test harness for the same reason, and are regenerated by a test rather than by curl:
...
and a chat session with history, which is more setup than a capture script should
own — and emitting them from the handlers is the same guarantee, since it is still
