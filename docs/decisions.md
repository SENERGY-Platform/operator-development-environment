...
| D33 | A session may hold a **standing answer** to `run_code`'s confirmation: auto mode runs code that `pkg/plaincode` recognises as inspection of data already in the kernel, and confirms everything else. It is relief from repeated prompting and **explicitly not a security boundary** — no sound static check exists for Python, and the boundary remains the developer's own pod and token. The recogniser is per tool, so no setting can waive a confirmation on a tool that has no recogniser; and `set_auto_run` is denied to the LLM, like `set_exposure_tier`. See [authorisation-and-exposure-tiers.md](authorisation-and-exposure-tiers.md). |
| D34 | A **failed** run's summary carries one bounded, tier-masked extract of its own last exception — class, innermost frames, message — and nothing else out of its output. §5.13's "never raw logs" is kept as the rule it was for: no log line reaches a model and no tool reads one (§5.8 is unchanged). What it is not is a claim that a run produced no output, which is what the injected message used to tell the model and the model then told the developer. The message is masked at **L0 and L1** and verbatim at **L2**, because a value in a traceback is a value and §3.2 puts values at L2; a credential-shaped string is withheld at every tier. The developer's own results route serves the extract unmasked — it is their data on their own token, and the whole log is one route further. Reasoning and the rejected alternatives in [result-interpretation.md](result-interpretation.md). |
| D35 | `run_code` is **contained by default** where `kernel_contain_cells` is on: a cell that did not ask for the platform token runs in a kernel that has none and is not confirmed; a cell that asked is confirmed always, and no setting waives it. The question moves from "is this recognisably an inspection", which Python cannot answer soundly and on which a recogniser tops out at 29% of real cells, to "does this cell need the credential", which the cell answers by running. It contains the credential and nothing else — egress belongs to the singleuser NetworkPolicy, which as deployed is open to public hosts, so the option is off by default. One kernel, not two, because the case it exists for is inspecting data a confirmed cell already fetched. Measurements, the rejected parser alternatives and the deployed policy in [authorisation-and-exposure-tiers.md](authorisation-and-exposure-tiers.md). |
| D36 | A session may carry a **data split**: a training end and a test end, developer-set, audited and denied as a tool, like the tier. The tier bounds *what kind* of data the assistant observes; the split bounds *up to when*. Every value read the model can cause passes through `timeseries.Query` with the split, which lowers a window's end to the training end and refuses a window that starts at or after it, so a profile or a preview cannot show the model what the training may not see. A launch from the session writes both bounds into the deployment config; Operator Lib v1.7.0 trains with its clock at the training end, replays `infer()` over `[training_end, test_end)` with the clock at each message, and records the bounds it applied as tags plus two artifacts. The library computes no metric and consults `need_retraining()` never; the summary compares the run's tags with the session's bounds and says **"not confirmed by the run"** when they are missing, which is how a cluster image on an older library shows. Reasoning in [authorisation-and-exposure-tiers.md](authorisation-and-exposure-tiers.md) and [experiments.md](experiments.md). |
| D37 | Under a data split, **the run the developer's own operator code writes to is not the run ODE reads, and what it wrote after training ends is withheld by its timestamp rather than by its name.** `MLOperator` ends the training run and drops `MLFLOW_RUN_ID` before the replay, then records the evaluation through `MlflowClient` against an explicit run id, so a fluent `log_metric()` inside `infer()` opens a run of its own instead of landing in ODE's; `searchRuns` keeps having no caller, because a search across the experiment would re-catch that foreign run. Because unsetting a variable takes nothing from code that read it at import time — `RUN = os.environ.get("MLFLOW_RUN_ID")` in `op.py`, then `MlflowClient().log_metric(RUN, ...)` in `infer()` — the library also stamps `operator_lib.training_ended_at` at the transition, and `MaskedFor` withholds every metric one of whose points is stamped at or after it, whatever its name: the claim is about a phase, so the filter is on the phase, and it reads the whole history because `log_metric` lets a caller choose both `step` and `timestamp`. The cutoff is itself on the run and therefore writable, so the filter is anchored on **ODE's own record of whether a split ran** — which the job cannot touch — and under a split a cutoff that is missing, unreadable or outside the run's own clock withholds every metric instead of falling back to the names. For the same reason params are cut to the replay's own four under a split, MLflow params carrying no timestamp to test, and the `data_split` block re-renders what the run recorded from the parsed instant instead of echoing the string. A write that forges a plausible timestamp still passes, which is named as a limit rather than presented as closed; the durable fix would be per-run write authorization, which MLflow as deployed does not offer. `evaluation/inputs.csv` is dropped from the other side, being platform measurements in a store the singleuser pod reaches without a token, where an unconfirmed cell could fetch them under `kernel_contain_cells`; the scoring script reads the actuals through timescale-wrapper on its own credential. Filtering the metric names against `evaluation.yaml` sits on top as **hygiene and not a boundary** — a declared name can carry a test-window value — with the count of everything withheld as `withheld_metrics` and never the names. What remains is named rather than closed: the exception text, which the tier governs (D34, verbatim at L2), and egress from the Ray cluster, which is the developer's own credential and outside ODE. Reasoning in [experiments.md](experiments.md). The same predicate reaches two fields that carry a metric's value without being the metrics map, both derived in `buildSummary` before `MaskedFor` runs and both open until an adversarial read found them: a graded `evaluation_criteria` — a declared metric logged from the replay was withheld from `metrics` and delivered anyway as the criterion's value and verdict — where the value now goes and `met` becomes `not_computed` with reason `metric_withheld` (D24 one level up: "this reader may not have it" is not "the run missed the target"), and `resource_usage.peak_memory_mb`, one float from a job-reported memory metric that needed no forgery at all. The developer's own route grades and reports both. |
