...
Plus the two platform timeouts, which exist for the same reason as the two-pass
read.

## Applies when

Adding a detector, a profile field, or a tool that returns a list of profiles;
changing what `profiler.Store` persists; or tuning either timescale-wrapper
timeout.
...
from SPEC D24 and §5.4.3; `mehrfach` for the budget figures, which came from
measured responses.
