...
- The SPA does not render the import half of a resolution: `SelectionResult` in `frontend/src/api.ts` and `frontend/src/__contract__/selection.json` do not carry `import_selectables`, `import_candidates` or `deployable_import_types`. Nothing breaks — the fixture still satisfies its type — but the fixture is no longer a complete record of the backend's answer.
