...
//
// It lives in a package of its own, and not in pkg/tools where it began, because
// two packages now enforce it. pkg/tools decides whether a tool may run at all,
// and pkg/experiments decides how much of a failed run's own output a model may
// read (D34) — and pkg/experiments cannot import pkg/tools, which imports it. A
// second vocabulary for the same ladder was the alternative, and it would have to
// be kept in step with this one by hand. The tier is a governance concept rather
// than a property of the tool layer, which is the reason it was ever a cycle.
//
