...
func TestTheScaffoldRendersEveryComplianceFileExceptTheLock(t *testing.T) {
	...
	if len(rendered) != len(repo.ScaffoldPaths())-1 {
		t.Errorf("rendered %d files, want %d", len(rendered), len(repo.ScaffoldPaths())-1)
	}
}

// The lock is resolved, not written from a template. A rendered one would be a
// fabricated resolution — every hash in it invented — and uv would either reject
// it or, worse, install from it.
func TestTheLockIsInTheComplianceSetButNeverRendered(t *testing.T) {
	if _, found := renderTestScaffold(t)[repo.LockFile]; found {
		t.Errorf("%s was rendered; it is generated by `uv lock`", repo.LockFile)
	}
	var inSet bool
	for _, path := range repo.ScaffoldPaths() {
		if path == repo.LockFile {
	...
	if !inSet {
		t.Errorf("%s is not in the compliance set, so nothing reports it missing", repo.LockFile)
	}
}
