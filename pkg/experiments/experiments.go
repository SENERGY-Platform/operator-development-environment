...
// Embeddable values for the framing probe of D6.
const (
	EmbedYes     = "yes"
	EmbedNo      = "no"
	EmbedUnknown = "unknown"
)

// EmbedProbe is one service's framing verdict (D6, §5.12).
//
// The backend half of the probe: it asks the service itself whether it permits
// framing, by reading X-Frame-Options and the frame-ancestors directive of a
// Content-Security-Policy. The frontend half — a hidden iframe with a load
// timeout — is the SPA's, because a header can permit framing while the page
// still refuses to render in one, and only a browser can find that out.
//
// "unknown" is a real answer and not a failure: a service ODE cannot reach may
// still be reachable from the developer's browser, so the pane tries the iframe
// and falls back to a link if it does not load.
type EmbedProbe struct {
	// Service is "ray" or "mlflow".
	Service    string `json:"service"`
	URL        string `json:"url"`
	Embeddable string `json:"embeddable"`
	// Reason is the header that decided it, or why nothing did. Carried because
	// "no" without the header is not actionable by whoever has to change it.
	Reason   string    `json:"reason"`
	ProbedAt time.Time `json:"probed_at"`
	// Status is the HTTP status the probe got. Zero when the service could not be
	// reached at all.
	Status int `json:"status,omitempty"`
}

// EmbedReport is both services, as one answer, with the cache's own age.
type EmbedReport struct {
	Services []EmbedProbe `json:"services"`
	// Cached says this answer came from the TTL cache rather than a fresh probe.
